...
여기서 중요한 점은 입력 데이터 자체가 파라미터는 아니라는 것이다. 모델은 데이터를 보고, 그 데이터를 더 잘 설명하는 내부 숫자들을 업데이트한다. Notion의 `파라미터란?` 문서에서 말한 “경사하강으로 업데이트되는 값들 전부”라는 정의가 가장 실용적이다.
...
Notion의 `Memory Caching` 문서는 이 차이를 잘 표현한다. RNN은 과거를 하나의 상태에 압축하는 쪽에 가깝고, Transformer attention은 과거 토큰의 정보를 직접 조회하는 쪽에 가깝다. 둘 중 어느 하나가 항상 정답이라기보다, 과거 정보를 어느 해상도로 저장하고 어떻게 검색할지가 핵심이다.
...
Notion의 `Attention & MLP 요약` 문서에서 말한 것처럼, Q/K는 “얼마나 볼지”를 정하고 V는 “무엇을 가져올지”를 담당한다. 그래서 attention은 정보를 섞는 라우팅 시스템에 가깝다.
...
여기서도 중요한 구분이 있다. embedding table이나 `W_Q`, `W_K`, `W_V` 같은 파라미터는 학습 후 추론 중에는 고정되어 있다. 그러나 각 토큰의 최종 표현과 attention weight는 입력 문맥에 따라 매번 달라진다. Notion의 `LLM 토큰 가중치는 동적인가?` 문서에서 말한 “초기 임베딩은 고정, 레이어를 지난 문맥 표현은 동적”이라는 설명이 이 지점이다.
...
- CNN 설명은 LeCun et al.의 LeNet/문서 인식 논문과 Notion의 CNN 문서 내용을 맞춰, 지역 수용영역과 가중치 공유 중심으로 정리했다.
...
- Notion: [Attention & MLP 요약](https://app.notion.com/p/330cf77c6633800b91cbf55dac7b2676)
- Notion: [transformer 모델](https://app.notion.com/p/260cf77c6633808eb352dc1b1996ac65)
- Notion: [CNN](https://app.notion.com/p/2accf77c6633800896d5c2f0109709d6)
- Notion: [CNN과 딥러닝](https://app.notion.com/p/2accf77c663380d780dfd90e56e62366)
- Notion: [Convolution Neural Networks (합성곱 신경망)](https://app.notion.com/p/2accf77c663381d5bb98c08c004555d2)
- Notion: [파라미터란?](https://app.notion.com/p/2accf77c663380cb8c34f1c2cd40e12b)
- Notion: [LLM 토큰 가중치는 동적인가?](https://app.notion.com/p/253cf77c663380ddb165d693bfc79130)
- Notion: [Memory Caching: RNNs with Growing Memory](https://app.notion.com/p/37acf77c6633809fb611f7c377479d84)
